...
echo "🔧 EAS Build Hook: Creating RNWorklets.podspec..."

WORKLETS_DIR="$EAS_BUILD_WORKINGDIR/node_modules/react-native-worklets"
PODSPEC_PATH="$WORKLETS_DIR/RNWorklets.podspec"

# Check if worklets dir exists
if [ ! -d "$WORKLETS_DIR" ]; then
  echo "❌ react-native-worklets not found at $WORKLETS_DIR"
  exit 0
fi

# Create the podspec file
cat > "$PODSPEC_PATH" << 'PODSPEC_EOF'
require "json"
require_relative './scripts/worklets_utils'

package = JSON.parse(File.read(File.join(__dir__, "package.json")))
$worklets_config = worklets_find_config()
# worklets_assert_minimal_react_native_version($worklets_config) # DISABLED
...
echo "✅ Created RNWorklets.podspec at $PODSPEC_PATH"
ls -la "$PODSPEC_PATH"
echo "🏁 EAS Build Hook complete"
